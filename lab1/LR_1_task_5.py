...
assert ozornin_f1_score(df.actual_label.values, df.predicted_RF.values) == f1_score(
    df.actual_label.values, df.predicted_RF.values
), "my_accuracy_score failed on RF"

print(
    "F1 RF: %.3f" % (ozornin_f1_score(df.actual_label.values, df.predicted_RF.values))
)
print(
    "Ozornin F1 LR: %.3f"
    % (ozornin_f1_score(df.actual_label.values, df.predicted_LR.values))
)
print("Lib F1 LR: %.3f" % (f1_score(df.actual_label.values, df.predicted_LR.values)))

# assert ozornin_f1_score(df.actual_label.values, df.predicted_LR.values) == f1_score(
#     df.actual_label.values, df.predicted_LR.values
# ), "my_accuracy_score failed on LR"
